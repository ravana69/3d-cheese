...
function spawnParticle(waveDirection) {
  ...
  vertex[5] = 0; //vy
  if (multiColor) {
    vertex[7] = ms/2+4+pi / 2 + pi / maxParticleSize * vertex[6]/1.5; //color
  } else {
    vertex[7] = -1;
  }
...
function render() {
  ...
  for (i = 0; i < particles.length; ++i) {
    ...
    particles[i][7]=ms/2+4+pi / 2 + pi / maxParticleSize * particles[i][6]/1.5;
  }
  ax /= particles.length;
  ay /= particles.length;
  az /= particles.length;
